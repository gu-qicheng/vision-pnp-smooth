...
逐帧处理会优先选择靠近上一帧的候选，并对角点做指数平滑。红框颜色短暂丢失时，程序用四角光流跟踪最多 3 帧，并在画面上标明跟踪状态；跟踪失败或超过 3 帧后清除旧姿态。
...
自动测试使用合成投影、合成红框、移动帧和短时颜色丢失帧验证 110 mm 对象点、PnP 恢复、角点排序、相机坐标逆变换、RMS、光流跟踪和跟踪超时清除；真实摄像头操作仍需要现场验收。
