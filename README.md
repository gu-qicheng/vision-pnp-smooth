...
当前版本使用 HSV 阈值和轮廓几何，不依赖 YOLO。每一帧独立检测；当前帧未检测到红框时清除姿态，不沿用旧坐标。
...
逐帧处理会优先选择靠近上一帧的候选，并对角点做指数平滑。候选突然跳到远处或消失时，当前帧标记为无效并清除旧姿态。

## 物理与坐标约定

- 正方形外轮廓边长：**105 mm（10.5 cm）**。
...
自动测试使用合成投影、合成红框和无目标帧验证 105 mm 对象点、坐标映射、PnP 恢复、角点排序、相机坐标逆变换、RMS 和标定输入校验；真实摄像头操作仍需要现场验收。
